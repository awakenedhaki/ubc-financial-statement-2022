...
async def scrape_employees(page, employee_names, start, end):
    results = defaultdict(list)
    for name in employee_names[start:end]:
        results_table = await scrape_employee_info(page, name)
        if results_table:
            results[name] = results_table
...
async def main(start, end):
    ...
    async with async_playwright() as p:
        ...
        results = await scrape_employees(page, employee_names, start, end)
        await browser.close()
    return results
